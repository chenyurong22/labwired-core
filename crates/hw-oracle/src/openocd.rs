...
impl OpenOcd {
            ...
            match TcpStream::connect(addr) {
                ...
                Err(_) => {
                    if Instant::now() >= deadline {
                        bail!("openocd TCL port 6666 did not become ready within 30 s");
                    }
    ...
    pub fn wait_until_halted(&mut self, timeout: Duration) -> Result<()> {
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                anyhow::bail!(
                    "wait_until_halted: timed out after {:.1}s",
                    timeout.as_secs_f32()
                );
            }
    ...
    pub fn wait_for_break(&mut self, timeout: Duration) -> Result<()> {
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                anyhow::bail!(
                    "wait_for_break: timed out after {:.1}s without BREAK",
                    timeout.as_secs_f32()
                );
            }
...
fn parse_hex_from_response(s: &str) -> Result<u32> {
    for token in s.split_whitespace() {
            ...
            .or_else(|| token.strip_prefix("0X"))
        {
            ...
            if !hex.is_empty() {
                return u32::from_str_radix(hex, 16).with_context(|| format!("parse hex '{hex}'"));
            }
        }
    }
...
mod tests {
    ...
    fn parse_mdw_response_multi_line() {
        ...
        let words = parse_mdw_response(resp, 4).unwrap();
        assert_eq!(words, vec![0xaabbccdd, 0x11223344, 0xdeadbeef, 0xcafebabe]);
    }
