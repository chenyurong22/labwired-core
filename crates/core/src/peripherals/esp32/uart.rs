...
impl Esp32Uart {
    ...
    /// CONF1 TXFIFO_EMPTY_THRHD[14:8] (7-bit on classic).
    fn txfifo_empty_thrhd(&self) -> usize {
        ((self.reg(OFF_CONF1) >> 8) & 0x7F) as usize
    }

    /// CONF1 RXFIFO_FULL_THRHD[6:0] (7-bit on classic).
    ...
    fn read_reg_word(&self, word_off: u64) -> u32 {
        match word_off {
            ...
            o => self.reg(o), // config storage or 0 for unmapped
        }
    }
    ...
    fn write_config_reg(&mut self, off: u64, value: u32) {
        if let Some(mask) = Self::is_config(off) {
            ...
            // Unknown config offset: round-trip so firmware RMW sees its writes.
...
impl Peripheral for Esp32Uart {
    ...
    fn write_u32(&mut self, offset: u64, value: u32) -> SimResult<()> {
        match offset & !3 {
            ...
            OFF_STATUS => {}
            o => self.write_config_reg(o, value),
        }
        Ok(())
    }
        ...
        if !self.tx_fifo.is_empty() {
            ...
            while self.drain_accum >= per_byte && !self.tx_fifo.is_empty() {
                ...
                if self.tx_fifo.is_empty() && self.tx_active {
                    self.int_raw_sticky |= INT_TX_DONE;
                    self.tx_active = false;
                }
            }
        } else {
...
mod tests {
    ...
    fn tx_fifo_fills_then_drains_to_sink() {
        ...
        assert_eq!((u.status_word() >> 16) & 0xFF, 3, "TXFIFO_CNT[23:16]=3");
        assert!(sink.lock().unwrap().is_empty(), "nothing shifted out yet");
        drain_all(&mut u);
        assert_eq!(sink.lock().unwrap().as_slice(), b"Hi!");
        assert_eq!((u.status_word() >> 16) & 0xFF, 0, "FIFO drained");
    }
    ...
    fn tx_done_raises_matrix_source_when_enabled() {
        ...
        u.push_tx(b'Q');
        let mut irq = None;
        for _ in 0..(u.cycles_per_byte() * 4) {
    ...
    fn conf0_reset_bits_flush_fifos() {
        ...
        u.write_u32(OFF_CONF0, CONF0_TXFIFO_RST | CONF0_RXFIFO_RST)
            .unwrap();
        assert_eq!(u.tx_fifo.len(), 0, "TX flushed");
        assert_eq!(u.rx_fifo.borrow().len(), 0, "RX flushed");
    }
